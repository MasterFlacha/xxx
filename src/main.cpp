...
// wczytywanie tekstu i zwrocenie go w tablicy wraz z dlugiscia
    ...
    //wynikowa tablica po encryption
    int des_exit_tab[64]{};
    for (int n = 0; n <32 ; ++n)
    ...
    for (int j = 0; j < 16; j++) {
        ...
        int subkey[48]{};
        int extended_left_tab[48]{};
        int temp[32]{};

        for (int i = 0; i < 32; i++)
            temp[i]=first_b_half[i];
        PermutationWithExtension(first_b_half,extended_left_tab);
        PermutationWithCompression(first_half_key,second_half_key,subkey);
        for (int k = 0; k < 48 ; k++)
            XOR(&extended_left_tab[k],&subkey[k]);
        sboxExit(extended_left_tab,first_b_half);
        for (int l = 0; l <32 ; l++)
            XOR(&first_b_half[l],&second_b_half[l]);
        for (int m = 0; m < 32 ; m++)
            second_b_half[m]=temp[m];
