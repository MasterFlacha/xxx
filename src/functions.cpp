...
void    BinaryToText (int Binary_tab[],char resultant_tab[], const int length_resultant_text) {
    ...
    for (int i = 0; i < length_resultant_text ; ++i) {
        resultant_tab[i] = BinaryToSign(&Binary_tab[Begin_8bit_tab]);
        Begin_8bit_tab+=8;
    }
...
void sboxExit(int *tab_48bit, int *exit_tab_32bit) {
    ...
    int temp[32];
    for (int j = 0; j <32  ; ++j) {
        temp[j]=exit_tab_32bit[j];
    }
    for(int i=0; i<32; i++)
    {
        exit_tab_32bit[i]=temp[p_block[i]];
    }
...
void BeginningPermutation (int *tab_64bit){
    int temp_tab[64]{};
    for (int i = 0; i < 64; ++i)
        temp_tab[i]=tab_64bit[recipe_for_beginning_permutation[i]];
    for (int j = 0; j < 64; ++j)
        tab_64bit[j]=temp_tab[j];
}

void    ReversePermutanion(int *tab_64bit){
    int temp_tab[64]{};
    for (int i = 0; i < 64; ++i)
        temp_tab[i]=tab_64bit[recipe_for_reverse_permutation[i]];
    for (int j = 0; j < 64; ++j)
        tab_64bit[j]=temp_tab[j];
}